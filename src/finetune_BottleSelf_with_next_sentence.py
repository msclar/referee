...
def computing_log_probability(model, input_ids, attention_mask):
    ...
    print('sizes', shift_logits.view(-1, shift_logits.size(-1)).shape, shift_labels.view(-1).shape)
    ...
    neglog_prob = (neglog_prob * attention_mask[:, 1:]).sum(axis=1)
...
def main(args):
    ...
    if args.max_sentences > 0:
        dataset_lines = dataset_lines[:args.max_sentences]
    ...
    if args.filter_by_next_sentence_probability:
        print('filter_by_next_sentence_probability')
        ...
        with torch.no_grad():
            ...
            dataset.to_csv(os.path.join(filtered_datasets_dir, 'nsp_score.csv'))

            # Easiest: filter by a constant. Problem: this penalizes longer stuff, since probability is naturally smaller
            # For now, we will remove sentences where diff_nsp_score >= 6. Shorter sentences should naturally have better NSP score
            # unless you removed something crucial!
            # BUGFIX: this used to be dataset = dataset.filter(lambda x: x['diff_nsp_score'] >= 6) which is WRONG! We want to *keep* <= 6
            print('pre_filter_nsp_score', dataset)
            dataset = dataset.filter(lambda x: x['diff_nsp_score'] <= args.max_diff_nsp_score)
            print('post_filter_nsp_score', dataset)
    ...
    filename = f"maxdiffnsp_{args.max_diff_nsp_score}{args.custom_token}_realnews_100k{'_filtered' if args.filter_dataset_based_on_nli else ''}_size_{train_size}_{model_type}_nepochs_{args.n_epochs}_lr_{args.learning_rate}_compression_{args.compression_rate}"
    print(filename)
