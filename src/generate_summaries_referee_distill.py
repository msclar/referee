    ...
    for filename in os.listdir(original_sentence_path):
        if file_regex.match(filename):
            ...
            with open(os.path.join(original_sentence_path, filename), 'r') as f:
                ...
                dataset_lines.extend([tmp[i] + f' {delim} ' for i in valid_ids])
...
def load_any_dataset(original_sentence_filename, delim, end_token, chunk_id_to_search, max_sentences=-1):
    dataset_lines = []
    valid_ids_dict = {}
    invalid_ids_dict = {}
...
def main(args):
    ...
    for chunk_id in range(args.min_chunk_id, args.max_chunk_id + 1):
        ...
        while upperbound < len(dataset):
            ...
            new_params['num_return_sequences'] = 1  # sample_output will be [num_return_sequences * batch_size, 2 * max_sentence_len]
